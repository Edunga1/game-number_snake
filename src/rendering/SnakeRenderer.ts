...
import { CELL_SIZE, COLOR_SNAKE_HEAD, COLOR_MERGE_GLOW } from '../constants';
...
export class SnakeRenderer {
  render(ctx: CanvasRenderingContext2D, snake: Snake, mergeSystem: MergeSystem) {
    ...
    for (let i = snake.segments.length - 1; i >= 0; i--) {
      ...
      const x = seg.pos.x * CELL_SIZE;
      const y = seg.pos.y * CELL_SIZE;
      const isHead = i === 0;
      ...
      // Body fill
      ctx.fillStyle = isHead ? COLOR_SNAKE_HEAD : getValueColor(seg.value);
